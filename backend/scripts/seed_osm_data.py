...
API: OpenStreetMap Overpass API (https://overpass-api.de/api/interpreter)

Pipeline Steps:
1. Query Overpass API for real restaurant, fast food, and cafe nodes in Lahore bounding box.
2. Extract fields: Name, Address/Suburb/Street, Phone Number, spatial Latitude & Longitude.
3. Build normalized 9-dimension taste vectors:
   [spicy, sweet, salty, sour, umami, smoky, creamy, crispy, rich] (0.0 to 1.0).
4. Insert/Upsert records into `public.restaurants` and construct PostGIS spatial points
   (POINT(lon lat)) in `public.restaurant_branches`.
5. Populate default menu categories & signature items.
6. Use SUPABASE_SERVICE_ROLE_KEY to bypass Row Level Security (RLS) policies.
7. Validate spatial indexing via PostGIS RPC `get_nearby_restaurants`.
"""
...
# 9-dimensional taste profiles for common cuisines:
# Dimensions: [spicy, sweet, salty, sour, umami, smoky, creamy, crispy, rich]
...
async def query_overpass_lahore() -> List[Dict[str, Any]]:
    ...
    headers = {
        "User-Agent": "FoodBookSeeder/1.0 (https://github.com/tahahaxan01-ai/FoodBook)",
    ...
    for server in OVERPASS_SERVERS:
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0, headers=headers) as client:
                res = await client.post(server, data={"data": query.strip()})
                if res.status_code == 200:
                    ...
                    # Try GET with query param if POST returned 406
                    res_get = await client.get(server, params={"data": query.strip()})
                    if res_get.status_code == 200:
                        data = res_get.json()
                        elements = data.get("elements", [])
                        if elements:
                            logger.info(f"Successfully retrieved {len(elements)} OSM nodes from {server} via GET")
                            return elements
        except Exception as e:
            logger.warning(f"Failed to fetch from {server}: {e}")

    logger.warning("Overpass online query unavailable or timed out. Using high-quality curated Lahore dataset.")
...
async def seed_restaurants_from_osm():
    ...
    1. Query OSM Overpass API for real restaurant nodes in Lahore.
    2. Extract fields: Name, Address/Suburb, Phone Number, and spatial Latitude/Longitude.
    3. Build normalized 9-dimension taste vectors.
    4. Insert records into `public.restaurants` and construct PostGIS spatial points
       (POINT(lon lat)) in `public.restaurant_branches`.
    5. Use SUPABASE_SERVICE_ROLE_KEY to bypass RLS policies.
    6. Verify PostGIS RPC `get_nearby_restaurants`.
    """
    logger.info("==================================================")
    logger.info("FoodBook OpenStreetMap (OSM) Overpass Data Seeder")
    logger.info(f"Target Database: {settings.SUPABASE_URL}")
    logger.info("Bypassing RLS with SUPABASE_SERVICE_ROLE_KEY")
    logger.info("==================================================")
    ...
    for elem in elements:
        ...
        try:
            ...
            if existing:
                ...
                # Insert Restaurant with 9-dim taste vector
                restaurant_data = {
                    "name": name,
                    "slug": base_slug,
                    "cuisines": cuisines,
            ...
            # 2. Insert Branch with PostGIS POINT(lon lat) coordinates
            ...
            if not existing_branches:
                ...
                seeded_branches += 1
                logger.info(f"   -> [CREATED] Branch: '{branch_name}' @ POINT({lon} {lat})")
            else:
            ...
            if not existing_categories:
                ...
                await db.insert("menu_items", {
                    ...
                    "category_id": cat_id,
                    "name": item_name,
                    "description": "House special preparation with authentic spices and fresh ingredients.",
                    "price": avg_price,
    ...
    try:
        # Test near Johar Town, Lahore
        test_lat = 31.4697
        test_lon = 74.2728
        test_radius = 20000  # 20km
        ...
        for idx, r in enumerate((rpc_res or [])[:5], start=1):
